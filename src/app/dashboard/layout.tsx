...
const Sidebar = ({ onClose }: { onClose?: () => void }) => {
  return (
    <div className="space-y-4 md:space-y-6 relative z-20 flex flex-col h-full">
      ...
      <Link
        href={"/"}
        className="hidden sm:block text-lg/7 font-semibold text-brand-900"
      >
  ...
  return (
    <div className="relative h-screen flex flex-col md:flex-row bg-white overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* mobile header */}
        <div className="md:hidden flex items-center h-16 justify-between border-b border-gray-200 px-5">
          <Link href={"/"} className="text-lg/6 font-semibold text-brand-900">
            App<span className="text-brand-700">Signal</span>
          </Link>
          <button
